...
void knight::updatePossibleMoves(std::pair<int, int> position){
    ...
    if(position.first - 2 >= 0){
        ...
        if(position.second - 1 >= 0){
            if(this->getGame()->getSquare(std::make_pair(position.first - 2, position.second - 1))->getPiece() == nullptr || this->getGame()->getSquare(std::make_pair(position.first - 2, position.second - 1))->getPiece()->getColor() != this->getColor()){
                this->getPossibleMoves()->push_back(std::make_pair(position.first - 2, position.second - 1));
            }
        }

        //check forward right case
        if(position.second + 1 <= 7){
            if(this->getGame()->getSquare(std::make_pair(position.first - 2, position.second + 1))->getPiece() == nullptr || this->getGame()->getSquare(std::make_pair(position.first - 2, position.second + 1))->getPiece()->getColor() != this->getColor()){
                this->getPossibleMoves()->push_back(std::make_pair(position.first - 2, position.second + 1));
            }
        }
    ...
    if(position.second - 2 >= 0){
        ...
        if(position.first - 1 >= 0){
            if(this->getGame()->getSquare(std::make_pair(position.first - 1, position.second - 2))->getPiece() == nullptr || this->getGame()->getSquare(std::make_pair(position.first - 1, position.second - 2))->getPiece()->getColor() != this->getColor()){
                this->getPossibleMoves()->push_back(std::make_pair(position.first - 1, position.second - 2));
            }
        }

        //check left backward case
        if(position.first + 1 <= 7){
            if(this->getGame()->getSquare(std::make_pair(position.first + 1, position.second - 2))->getPiece() == nullptr || this->getGame()->getSquare(std::make_pair(position.first + 1, position.second - 2))->getPiece()->getColor() != this->getColor()){
                this->getPossibleMoves()->push_back(std::make_pair(position.first + 1, position.second - 2));
            }
        }
