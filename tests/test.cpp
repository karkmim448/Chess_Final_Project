...
TEST(ABSTRACT_FACTORY_PATTERN, BLACK_PIECE){
  BlackPieceFactory test;
  piece* p = test.DrawPawn(std::make_pair(0,0));
  ASSERT_EQ(p->getColor(), false);
}

TEST(ABSTRACT_FACTORY_PATTERN, WHITE_PIECE){
  WhitePieceFactory test;
  piece* p = test.DrawPawn(std::make_pair(0,0));
  ASSERT_EQ(p->getColor(), false);
}
